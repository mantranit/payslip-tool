...
def get_data_row(row, password):
    return {
        ...
        'advance': get_int(row.get('Advance (8)', 0)),
        'otherAllowance': get_int(row.get('Other (9)', 0)),
        'overtimePay': get_int(row.get('Overtime pay (10)', 0)),
        'totalSalary': get_int(row.get('Total salary (11) = (4)+(5)+(6)+(7)+(8)+(9)+(10)', 0)),
        'insurance': get_int(row.get('Insurance: Social, health, unemployment (10.5% x 4.729.400đ) (12)', 0)),
        'incomeTax': get_int(row.get('Personal Income Tax (13)', 0)),
        'deduct': get_int(row.get('Deduct (14)', 0)),
        'netAmount': get_int(row.get('Net Amount (15) = (11) - (12) - (13) - (14)', 0)),
